...
const ConfirmSwap = () => {
  ...
  const filteredTokens = tokenData.filter((e) => e.name.toLowerCase().includes(searchTerm.toLowerCase()))
    ...
    async (rfsObject) => {
      try {
        ...
        if (coingeckoCachedResponse) {
          ...
          token0 = tokens.filter((token) => rfsObject.token0 === ethers.utils.getAddress(token.address))[0]
        } else {
          ...
          const token0 = tokens.filter((token) => rfsObject.token0 === ethers.utils.getAddress(token.address))[0]
        }
  ...
  const getRfsData = async () => {
    ...
    const otcNexus = new ethers.Contract(networkMapping[chainId].OtcNexus, OtcNexusAbi, provider)
  ...
  const handleSwap = async () => {
    ...
    let tx
    if (rfs.typeRfs === 0) {
      tx = await otcNexus.takeDynamicRfs(rfs.id, ethers.utils.parseUnits(amountToBuyRef.current), index)
    } else {
            ...
            {rfs &&
              filteredTokens?.map((e, i) => {
                return (
                  ...
                  >
                    <Image src={e?.logoURI} alt="logo" height={10} width={30} className="rounded-full mr-1 p-1" />
                    <div className=" font-montserrat ml-2 font-medium text-sm">{e?.name}</div>
                    <div className=" ml-2 text-xs font-light text-[#51596f]">{e?.symbol}</div>
                  </div>
                )
              })}
          </div>
          ...
          <div className="w-full flex h-full justify-center items-center">
            <div className="w-[500px] bg-gray-950 bg-opacity-20 shadow-lg border-opacity-18 border border-gray-600 h-fit rounded-2xl flex flex-col px-10 z-10">
              ...
              <div className="relative mb-3">
                ...
                />
                <Input type="text" placeholder="0" pattern="^-?\d*[.,]?\d*$" value={tokenTwoAmount} disabled={true} />
                ...
                <VStack align="start" className=" mb-5 mt-5">
                  <h2 className=" font-montserrat font-bold text-lg text-white te">Current swap price :</h2>
                ...
                <VStack align="start" className=" mb-5 justify-start ">
                  <h2 className=" font-montserrat font-bold text-ml text-white">Maker Address :</h2>
                  <p className="font-montserrat font-medium text-sm text-gray-400">{rfs.maker}</p>
                </VStack>
                {rfs.priceMultiplier != 0 && (
                  <HStack className=" mb-5">
                    <h2 className=" font-montserrat font-bold text-lg text-white">Discount/Premium :</h2>
                    <p
                    ...
                    >
                      {rfs.priceMultiplier > 100 ? `+${rfs.priceMultiplier - 100}` : `-${100 - rfs.priceMultiplier}`}%
                    </p>
                  </HStack>
                )}
                {error && (
                  <div className="w-full flex justify-center mb-5 mt-5">
                    <p className="text-red-500 font-montserrat font-bold text-sm">{error}</p>
                  </div>
                )}
                <div className="w-full flex justify-center mb-5 mt-5">
                  <Button onClick={handleSwap} className="bg-blue-gradient rounded-xl h-fit w-fit py-2  px-16">
                    Swap
                  </Button>
                  <Button onClick={handleApprove} className="bg-blue-gradient rounded-xl h-fit w-fit py-2  px-14">
                    Approve
                  </Button>
                </div>
