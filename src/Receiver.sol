...
contract Receiver {
  ...
  modifier onlyConsumer() {
    require(msg.sender == address(functionsConsumer), "Only the consumer can call this function");
    _;
  }

  constructor(uint32 _gasLimit, address _otcSwapProtocol, uint64 _subscriptionId, address _functionsConsumer) {
  ...
  function executeMetaTransaction(
    ...
    for (uint256 i = 0; i < transactions.length; i++) {
      ...
      (bytes memory clientId, bytes memory clientSecret) = abi.decode(secrets[i], (bytes, bytes));
      ...
      capturePayment(0, rfsId, makerAddress, paypalEmail, amountToBuy, "EUR", nonce, clientId, clientSecret);
    }
  }
  ...
  function performSwaps(bytes calldata order_id) public onlyConsumer {
    ...
    otcSwapProtocol.performSwaps(transaction.rfsId, transaction.amountToBuy, transaction.makerAddress);
